...
2. **原文中出现的所有数字必须按原始字面保留**：数值、百分比、倍数、单位一个都不能丢，**数字本身的写法也不许改**（例：原文 `9000+` 就保留 `9000+`，不要缩成 `9k+`；原文 `10,000+` 保留 `10,000+`，不要改成 `10k+` 或 `1w+`）。只允许调整加粗范围，不允许改数字的表达形式。
3. **bullet 数量** 保持不变
4. **不编造** 任何未在原素材中出现的项目、数据、技能
5. 只改：职位 title 朝 JD 方向靠、bullet 措辞和关键词、profile 总结语
6. **加粗统一用 HTML `<b>...</b>` 标签，禁用 Markdown `**...**`**
